...
class Rating(models.Model):
    session = models.ForeignKey("MatchSession", on_delete=models.CASCADE, related_name="ratings")
...
class TeamAssignment(models.Model):
    session = models.ForeignKey("MatchSession", on_delete=models.CASCADE, related_name="team_assignments")
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE)
    team = models.PositiveIntegerField()  # 1 or 2
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def __str__(self):
        return f"{self.session_id}: {self.user_id} -> Team {self.team}"
